...
const CoursesHome = () => {
    ...
    const [courses, setCourses] = useState<IcourseData[]>([]); // Use IcourseData type
    ...
    const fetchCourses = useCallback(async () => {
        ...
        try {
            ...
            if (Array.isArray(response.data.courses)) {
                setCourses(response.data.courses); // Directly set the courses
            } else {
    ...
    return (
        <section className='container pl-10 py-16 bg-[#f2f2f2]' id="courses">
            ...
            {error && <p className="text-red-500 text-center">Error: {error}</p>} {/* Display error message */}
